    ...
    <div id="kt_content_container" class="container-xxl">
        ...
        @method('PUT')
            <div class="d-flex flex-column gap-3 gap-lg-7 w-100 mb-2 me-lg-5">
                <div class="card card-flush py-0">
                    <div class="card-body pt-0">
                        <div class="d-flex flex-column gap-5 mt-5">
                            <x-backend.cms.masterInputs :showDescription="1" :richTextArea="1" :showSlug="1" :row="$row" />

                            <div class="fv-row fl">
                                <label class="form-label"
                                    for="sub_title">{{ __('site.sub_title') }}</label>
                                <input placeholder="{{ __('site.sub_title') }}" type="text" id="sub_title"
                                    name="sub_title" class="form-control mb-2" value="{{ $row->sub_title ?? '' }}" />
                            </div>
                ...
                <x-backend.cms.blocks :blocks="$blocks" :row="$row" />
...
@push('scripts')
    ...
    <script>
        ...
        KTUtil.onDOMContentLoaded(function() {
            handleFormSubmitFunc('Edit{{ $trans }}','richtext');
        });
    </script>
@endpush
